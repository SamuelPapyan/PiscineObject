...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/10 21:50:39 by spapyan           #+#    #+#             */
/*   Updated: 2025/08/10 21:50:39 by spapyan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    Bank::close_account(int id)
{
    ...
    if ((it = accounts_db.find(id)) == accounts_db.end())
        throw MyRuntimeError("This given account id doesn't exists");
...
void    Bank::deposit_money(int id, int amount)
{
    ...
    if (amount <= 0)
        throw MyRuntimeError("invalid amount!, your amount shoudl be a positive value");
    fee = amount * 0.05;
    this->liquidity += fee;
...
void    Bank::withdraw_money(int id, int amount)
{
    ...
    if (amount > it->second->value)
        throw MyRuntimeError("rejected!, insufficient fund for withdraw");
